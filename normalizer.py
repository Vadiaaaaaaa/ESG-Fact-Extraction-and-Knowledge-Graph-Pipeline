...
def _aliases_with_seed() -> dict[str, str]:
    aliases_lookup = {
        ...
        for key, value in _load_json("registry_aliases.json").items()
    }
...
def _metric_registry_path() -> Path:
    preferred = Path("consumer_master_registry_v1.json")
    if preferred.exists():
        return preferred
    return Path("metric_registry.json")
...
def _preflight_check(pass1_path: str | Path) -> list[dict[str, Any]]:
    ...
    aliases_path = Path("registry_aliases.json")
