...
# Cost rates per million tokens — update here when model changes
MODEL_COSTS: dict[str, dict[str, float]] = {
    "gpt-4.1-mini":  {"prompt_per_million": 0.40,  "completion_per_million": 1.60},
    "gpt-4.1":       {"prompt_per_million": 2.00,  "completion_per_million": 8.00},
...
def _parse_with_truncation_recovery(content: str, chunk_id: str) -> dict:
    """Parse JSON response, attempting partial recovery if truncated."""
    try:
        return json.loads(content)
    except json.JSONDecodeError:
        print(
            f"WARNING: Response appears truncated at {len(content)} chars for {chunk_id}, "
            "attempting recovery",
            flush=True,
        )
        # Find the last complete fact object — scan backwards for "}," or lone "}"
        recovered = None
        # Try to close the facts array and wrapper object after the last complete item
        for end_marker in ("},\n", "},", "}"):
            pos = content.rfind(end_marker)
            if pos == -1:
                continue
            # Include the closing "}" but not any trailing comma
            cut = pos + 1  # include the "}"
        ...
        if recovered is not None:
            n_recovered = len(recovered.get("facts", []))
            print(
                f"Recovery succeeded: {n_recovered} facts recovered for {chunk_id}",
                flush=True,
            )
            return recovered
        print(
            f"Recovery failed for {chunk_id}, returning empty facts",
            flush=True,
        )
        return {"facts": []}
...
def _call_openai(chunk: Chunk) -> Any:
            ...
            "json_schema": {
                ...
                "schema": {
                    ...
                    "fact_type": {"type": "string"},
                                    ...
                                    "normalised_unit_symbol": {"type": "string"},
    ...
    response_json = _parse_with_truncation_recovery(content, chunk.chunk_id)
...
def run_pass1(
    ...
    partial_path = output_path.with_name(output_path.stem.replace("_pass1", "") + "_pass1_partial.jsonl")
    ...
    # Load already-completed chunks from partial file (crash recovery)
    completed_chunk_ids: set[str] = set()
    facts: list[ExtractedFact] = []
    if partial_path.exists():
        print(f"Resuming from partial file: {partial_path}", flush=True)
        with open(partial_path, "r", encoding="utf-8") as _pf:
            for _line in _pf:
                _line = _line.strip()
                if not _line:
                    continue
                try:
                    ...
                    cid = _rec.get("chunk_id", "")
                    completed_chunk_ids.add(cid)
                    for _fd in _rec.get("facts", []):
                        facts.append(ExtractedFact.from_dict(_fd))
                except Exception:
                    pass
        print(f"Recovered {len(facts)} facts from {len(completed_chunk_ids)} completed chunks", flush=True)
        chunks_to_process = [c for c in chunks_to_process if c.chunk_id not in completed_chunk_ids]
        print(f"Remaining chunks to process: {len(chunks_to_process)}", flush=True)
    ...
    with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_CALLS) as executor:
        ...
        for future in as_completed(futures):
            ...
            for index, raw_fact in enumerate(_extract_fact_items(response_json or {}), start=1):
                fact = _to_extracted_fact(chunk, raw_fact, index)
                chunk_facts.append(fact)
                facts.append(fact)

            # Append chunk record to partial file for crash recovery
            with _partial_lock:
                with open(partial_path, "a", encoding="utf-8") as _pf:
                    _pf.write(json.dumps({
                        "chunk_id": chunk.chunk_id,
                        "facts": [f.to_dict() for f in chunk_facts],
                    }, ensure_ascii=False) + "\n")
    ...
    # Clean up partial file now that final output is written
    if partial_path.exists():
        partial_path.unlink()
        print(f"Partial file removed: {partial_path}", flush=True)
    ...
    # Write cost summary
    rates = MODEL_COSTS.get(MODEL, MODEL_COSTS["gpt-4.1-mini"])
    prompt_cost = _token_usage["prompt"] / 1_000_000 * rates["prompt_per_million"]
    completion_cost = _token_usage["completion"] / 1_000_000 * rates["completion_per_million"]
    total_cost = prompt_cost + completion_cost
    cost_summary = {
        "doc_id": Path(output_path).stem.replace("_pass1", ""),
        "chunks_processed": len(chunks_to_process),
        "prompt_tokens": _token_usage["prompt"],
        "completion_tokens": _token_usage["completion"],
        "cost_usd": {
            "prompt": round(prompt_cost, 4),
            "completion": round(completion_cost, 4),
            "total": round(total_cost, 4),
        },
        "model": MODEL,
        "rates": rates,
    }
    cost_path = Path(output_path).with_name(Path(output_path).stem.replace("_pass1", "") + "_cost_summary.json")
    with open(cost_path, "w", encoding="utf-8") as f:
        json.dump(cost_summary, f, indent=2)
    print(
        f"API cost: ${total_cost:.2f} "
        f"({_token_usage['prompt']//1000}K prompt + {_token_usage['completion']//1000}K completion tokens)",
        flush=True,
    )
