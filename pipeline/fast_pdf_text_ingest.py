...
ESG_UNIT_PATTERNS = [
    r'\d[\d,\.]+\s*(?:tCO2e|tco2e|kL|kilolitre|MJ|GJ|TJ|kWh|MWh|MT\b)',
    r'(?:metric\s+ton|metric\s+tonne|kilolitre|gigajoule|megajoule)',
    r'\d[\d,\.]+\s*(?:kt\s|Kt\s)',
    r'scope\s+[123]\s+emission',
    r'water\s+withdrawal.*\d',
    r'energy\s+consumption.*\d',
]


def esg_unit_score(text: str) -> int:
    score = 0
    for pattern in ESG_UNIT_PATTERNS:
...
def select_operational_pages(
    ...
    *,
    ...
    selected = [
        ...
        and (
            (page["has_numeric_metric"] and page["include_score"] >= min_score)
            or esg_unit_score(page["text"]) >= 3
        )
    ]
...
def main() -> None:
    ...
    parser.add_argument("--keyword-threshold", type=int, default=3)
    ...
    section_selection = find_operational_sections(args.pdf, keyword_threshold=args.keyword_threshold)
