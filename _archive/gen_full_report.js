...
async function main() {
  ...
  await Packer.toBuffer(nestleDoc).then(buf => fs.writeFileSync('nestle_india_full_report.docx', buf));
  console.log('Done: nestle_india_full_report.docx');

  await Packer.toBuffer(tataDoc).then(buf => fs.writeFileSync('tata_consumer_full_report.docx', buf));
